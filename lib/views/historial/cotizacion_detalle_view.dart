...
class CotizacionDetalleView extends ConsumerStatefulWidget {
  const CotizacionDetalleView({super.key, this.ids});

  final Tuple2<String?, int?>? ids;

  @override
  ConsumerState<CotizacionDetalleView> createState() =>
      _CotizacionDetalleViewState();
}
...
class _CotizacionDetalleViewState extends ConsumerState<CotizacionDetalleView> {
  ...
  Widget build(BuildContext context) {
    final cotizacionDetalle = ref.watch(cotizacionByIdProvider(widget.ids));
    double screenHight = MediaQuery.of(context).size.height;
    double screenWidth = MediaQuery.of(context).size.width;
    final sidebarController = ref.watch(sidebarControllerProvider);

    double screenWidthWithSideBar = screenWidth +
        (screenWidth > 800 ? (sidebarController.extended ? 50 : 180) : 300);

    return PageBase(
      children: [
        cotizacionDetalle.when(
          data: (cotizacion) {
            if (cotizacion == null) return const MessageErrorScroll();
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    if (cotizacion.cliente?.correoElectronico ==
                                            null &&
                                        cotizacion.cliente?.numeroTelefonico ==
                                            null)
                                      Row(
                                        crossAxisAlignment:
                                        ...
                                        Expanded(
                                          child: TextFormFieldCustom
                                            ...
                                            name: (cotizacion.estatus ==
                                                ...
                                                ? "Responsable"
                                                : "Fecha de vigencia",
                                            initialValue: (cotizacion.estatus ==
                                                    "concretado")
